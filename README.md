...
阶段一仍按 `INTAKE → DIVERGE → SEARCH → COMPARE → DRAFT → CONFIRM → GATE_CHECK` 推进，但 `DIVERGE` 内部是“理解—完善—替代检查—侦察—修订”循环，`SEARCH` 只验证正式入围方向，`DRAFT` 在唯一方向确认后才补齐数据、baseline、指标、成功标准和实验设计。普通回合只更新 `RESEARCH.md` 的精简活动区；完整合同和门禁只在检查点同步。
...
```text
/plan 阅读 RESEARCH.md；请先通过高信息量问题完善我的工作方向，检查是否存在实质替代方向，并仅在可能改变方向去留时进行轻量侦察。方向入围后再正式搜索；由你总结和推荐、我确认唯一方向后，再补齐数据、评测和阶段二资源方案。
```
