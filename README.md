...
阶段一仍按 `INTAKE → DIVERGE → SEARCH → COMPARE → DRAFT → CONFIRM → GATE_CHECK` 推进。最初 1–3 轮 High 进行“理解—完善—替代检查—必要侦察—修订”的方向推理；方向基本明确时可以展示方法级草案与关键取舍供用户讨论，但不提前形成正式实验设计。High 期间不修改项目文件、不分配 ID、不正式搜索，也不运行验证。方向交接后切换 Medium；由 Medium 一次性更新 `RESEARCH.md`，随后完成正式搜索、比较、设计和门禁。最终方向在标记为 `已选定` 前必须向用户展示并取得明确确认。
...
初始的 1–3 轮头脑风暴使用 **GPT-5.6 Sol / High**，处理开放式问题定义、研究空白、替代方向与方案取舍；方向基本明确后，可以给出不含正式对象和资源承诺的方法级草案供用户交流。High 期间不修改仓库、不创建稳定 ID 或 TODO、不进行正式检索与证据登记、不补正式数据/评测/实验方案，也不运行验证器或测试。方向交接后切换到 **GPT-5.6 Sol / Medium**，由 Medium 一次性持久化必要结果并完成阶段一其余工作。

可先使用普通对话或 `/plan`，例如：

```text
/plan 只读取 RESEARCH.md 中理解当前起点所需的状态；在 High 的 1–3 轮内通过高信息量问题完善工作方向、检查实质替代方向，并仅在可能改变方向去留时进行轻量侦察。方向基本明确时可展示方法级草案和关键取舍供我讨论，但不要修改文件、创建 ID 或 TODO、正式搜索、登记证据或形成正式数据/评测/实验/资源方案。方向交接后提示我切换 Sol / Medium；切换后再一次性写入 RESEARCH.md、正式搜索和总结推荐。将唯一方向标记为已选定前，向我展示拟定方向、核心问题、范围边界和方法概览并取得明确确认；确认后再补齐研究设计和阶段二执行方案。
```

模型切换规则本身不能保证客户端自动更换模型。如果当前界面不支持自动切换，High 应在交接摘要后停止，由用户切换模型或新开 Medium 会话后继续。
