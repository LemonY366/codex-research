...
`validate_workspace.py` 是模板预置的只读检查工具，用于交叉检查机器状态、转换账本和 `RESEARCH.md` 投影，并检查紧凑方向字段、Codex 主导头脑风暴、替代方向检查、正式入围方向与唯一最终方向、“证据饱和/可行性阻断”搜索停止证据、合同字段确认依据、数据证据成熟度、恢复摘要、稳定 ID、研究问题一致性关系、阶段二任务回指，以及 `research/` 证据。进入 `GATE_CHECK` 时，合同、证据、恢复摘要和未解决 TODO 都按完整门禁检查；更早子状态的严格验证通过不代表阶段一已完成。工具还检查阶段 TODO、registry 与本地运行对账、运行可复现字段、阶段二/三任务进度与心跳、证据交接和论文实验 ID 追溯；它不启动 Codex、不读取 `.env`、不创建或修复文件，也不替代来源真实性和科研有效性判断。
