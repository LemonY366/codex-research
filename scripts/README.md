...
常规模式在发现错误时返回非零状态；`--strict` 也将警告视为失败。`tests/` 使用 Python 标准库 `unittest` 覆盖阶段一合同、门禁、证据引用、安全与迁移行为。质量预警和凭据检查是启发式检查，不能代替科研合理性评审、来源事实核验或专用密钥扫描器。运行证据的覆盖检查可识别 Git 已跟踪文件的改写；默认忽略、未建立外部校验基线的本地运行无法仅凭当前文件状态证明从未被修改。
