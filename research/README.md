...
文件使用 UTF-8 JSON Lines；每个非空行必须是一个完整 JSON 对象，不允许跨行、注释或尾随逗号。Medium 在 High 交接后执行的每次正式查询对应一行，查询 ID 从 `QRY-001` 开始，数字部分至少三位且增长时可以超过三位。High 为方向去留进行的临时侦察不写入本文件，也不能直接支撑合同；需要采用时由 Medium 重新核验并登记。记录完成后按追加式证据处理；需要修正时追加新的查询记录，并通过可选的 `supersedes_query_id` 指向旧记录，不原地美化历史查询。
